...
**Gotcha — Atlantis cannot replace the NAT gateway it breathes through.** NAT replacement is destroy-then-create; the instant the old NAT dies, the Atlantis pod loses all egress — it can't call AWS to create the replacement, can't reach S3 state, can't even report failure to GitHub. Run NAT/VPC-egress changes from outside the cluster (break-glass laptop), or add VPC endpoints for S3/STS/EC2 so terraform survives NAT loss. Related hard rule: **never `force-unlock` while the locking process may still be alive** — a stalled Atlantis apply resumed when egress returned and overwrote state written in the meantime; the fix was importing the orphaned resources. Prefer waiting out the lock, or kill the Atlantis pod first.

**Access entries, resolved.** `enable_cluster_creator_admin_permissions` is now `false` and every kubectl principal is declared in `operator_principal_arns` — caller-independent config, no more flip-flop. This is the day-one posture the real project should copy verbatim.
